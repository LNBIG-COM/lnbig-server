...
const TARGET_FEE_MARKET = 3000

// Комиссия между нашими узлами
const TARGET_FEE_OUR_NODE = 0

const POLICY_FOR_OUR_NODES = {
    base_fee_msat: 0,
    fee_per_mil: TARGET_FEE_OUR_NODE,
    fee_rate: TARGET_FEE_OUR_NODE / FEE_DENOMINATOR
}

const POLICY_FOR_FOREIGN_NODES = {
    base_fee_msat: 900,
    fee_per_mil: TARGET_FEE_MARKET,
    fee_rate: TARGET_FEE_MARKET / FEE_DENOMINATOR
}
    ...
    for (let channel of channels) {
        ...
        if (channel.channel.private) {
            ...
            if (feeNotChanged(data, channel) || program.dryRun) {
                ...
                console.log(`Обновление private fee канала: ${channel.key}/${data.chan_point.funding_txid_str}:${data.chan_point.output_index} ${channel.fee.base_fee_msat}->${data.base_fee_msat}/${channel.fee.fee_per_mil}->${data.fee_per_mil}`)
            ...
            if (feeNotChanged(data, channel) || program.dryRun) {
                ...
                console.log(`Обновление my own fee канала: ${channel.key}/${data.chan_point.funding_txid_str}:${data.chan_point.output_index} ${channel.fee.base_fee_msat}->${data.base_fee_msat}/${channel.fee.fee_per_mil}->${data.fee_per_mil}`)
                yield nodeStorage.nodes[channel.key].client.updateChannelPolicy(data)
                    .catch(e => {
                        console.log("updateChannelPolicy ОШИБКА (@%s): %o\nОШИБКА: %s", channel.key, data, e.message)
                    })
            ...
            if (lbPart >= LOWER_LOCAL_BALANCE && lbPart <= UPPER_LOCAL_BALANCE) {
                ...
                if (feeNotChanged(data, channel) || program.dryRun) {
                    ...
                    console.log(`Обновление fee канала: ${channel.key}/${data.chan_point.funding_txid_str}:${data.chan_point.output_index} ${channel.fee.base_fee_msat}->${data.base_fee_msat}/${channel.fee.fee_per_mil}->${data.fee_per_mil}`)
                    yield nodeStorage.nodes[channel.key].client.updateChannelPolicy(data)
                        .catch(e => {
                            console.log("updateChannelPolicy ОШИБКА (@%s): %o\nОШИБКА: %s", channel.key, data, e.message)
                        })
