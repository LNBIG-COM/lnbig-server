...
function printReport(forwardingHistory) {
    ...
    _.forEach(forwardingHistory, (res, key) => {
        debug("(%s) length array=%d, last_offset_index=%d", key, res.forwarding_events.length, res.last_offset_index)
        amount += res.forwarding_events.length
        _.forEach(res.forwarding_events, item => {
            sat += item.amt_in
        })
    })
    console.log("The last 24 hours of LNBIG:\nAmount transactions: %i", amount)
    console.log("The sum of transfered BTC: %f", sat / 100000000)
}
